    ...
    *,
    ...
    try:
        ...
        raise ReadinessCheckError(
            ...
            "supplier database is unavailable",
        ) from exc
    ...
    try:
        ...
        raise ReadinessCheckError(
            ReadinessState(database="up", migrations="unknown", kafka="unknown"),
            "migration metadata is unavailable",
        ) from exc

    if current_revision != expected_revision:
        raise ReadinessCheckError(
            ReadinessState(database="up", migrations="out_of_date", kafka="unknown"),
            "database migration revision is out of date",
        )

    workers_enabled = (
        application_settings.reservation_consumer_enabled
        or application_settings.supplier_outbox_publisher_enabled
    )
