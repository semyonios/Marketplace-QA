...
class KafkaDeliveryError(RuntimeError):
    """Kafka did not acknowledge delivery within the configured timeout."""


class JsonMessageProducer(Protocol):
    def publish(
        self,
        *,
        topic: str,
...
class ConfluentJsonProducer:
    ...
    def publish(
        ...
        *,
        ...
        delivery_error: list[KafkaDeliveryError | None] = [None]
        ...
        if headers is not None:
            kafka_headers = [
                (name, None if item is None else str(item).encode("utf-8"))
